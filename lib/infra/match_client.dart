...
final matchedUserProfileProvider = Provider<MatchedUserProfileHandler>(
    (ref) => MatchedUserProfileHandler(ref));
...
class MatchedUserProfileHandler {
  ...
  getMatchedUsers(offset, limit) async {
    ...
    if (ref.read(isMatchedUserListMoreProvider)) {
      final res = await dio.get("/api/match/v1/users",
          queryParameters: {"offset": offset, "limit": limit});

      MatchedUserProfilesView item = MatchedUserProfilesView.fromJson(res.data);

      // 더보기 버튼 유출 여부
      if (item.paging.next == null)
        ref
            .read(isMatchedUserListMoreProvider.notifier)
            .update((state) => state = false);

      ref
          .read(matchedUserListProvider.notifier)
          .update((state) => state = [...state, ...item.list]);
...
// 매칭된 유저 리스트
final matchedUserListProvider =
    StateProvider<List<UserProfileView>>((ref) => []);
...
// 매칭 유저 상세 정보 프로바이더
final matchedUserDetailProvider =
    StateProvider<UserProfileView?>((ref) => null);
